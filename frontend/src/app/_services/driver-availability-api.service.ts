...
import type { DriverAvailabilityDto, UpsertAvailabilityRequest } from '@models/driver.model';
...
export class DriverAvailabilityApiService {
  ...
  getAvailability(
    ...
    const params = new HttpParams().set('from', fromYmd).set('to', toYmd);
    return this.http.get<DriverAvailabilityDto[]>(`${this.baseUrl}/${toolId}/availability`, {
      params,
    });
  }
  ...
  deleteAvailability(toolId: string, dateYmd: string): Observable<void> {
    return this.http.delete<void>(`${this.baseUrl}/${toolId}/availability/${dateYmd}`);
  }
}
