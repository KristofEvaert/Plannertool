...
export class UsersPage {
  ...
  loadUsers(): void {
    ...
    this.usersApi.getUsers().subscribe({
      ...
      error: (err) => {
        this.loading.set(false);
        this.messageService.add({ severity: 'error', summary: 'Error', detail: err?.message || 'Failed to load users' });
      },
    });
  }
  ...
  createUser(): void {
    if (!this.createEmail || !this.createPassword) {
      this.messageService.add({ severity: 'warn', summary: 'Validation', detail: 'Email and password required' });
      return;
    }
    this.loading.set(true);
    this.usersApi.createUser({
      ...
      error: (err) => {
        this.loading.set(false);
        this.messageService.add({ severity: 'error', summary: 'Error', detail: err?.error?.message || err?.message || 'Failed to create user' });
      },
    });
  }
  ...
  assignRoles(user: UserDto): void {
    ...
    const ownerIdForDriver = roles.includes('Driver') ? this.driverOwnerSelection[user.id] : undefined;
    if (roles.includes('Driver') && (!ownerIdForDriver || ownerIdForDriver <= 0)) {
      this.messageService.add({ severity: 'warn', summary: 'Validation', detail: 'Select an owner for driver role' });
      return;
    }
    ...
    const hasStartLatitude = startLatitude !== null && startLatitude !== undefined && startLatitude !== 0;
    const hasStartLongitude = startLongitude !== null && startLongitude !== undefined && startLongitude !== 0;
    if (roles.includes('Driver')) {
      if (hasStartLatitude !== hasStartLongitude) {
        this.messageService.add({ severity: 'warn', summary: 'Validation', detail: 'Provide both latitude and longitude, or leave both empty' });
        return;
      }
      if (!hasStartAddress && !hasStartLatitude) {
        this.messageService.add({ severity: 'warn', summary: 'Validation', detail: 'Provide start address or start coordinates for driver' });
        return;
      }
    }
    ...
    if (needsStaffOwner && (!ownerIdForStaff || ownerIdForStaff <= 0)) {
      this.messageService.add({ severity: 'warn', summary: 'Validation', detail: 'Select an owner for admin/planner' });
      return;
    }
    this.loading.set(true);
    this.usersApi.assignRoles({
      userId: user.id,
      roles,
      ownerIdForDriver,
      ownerIdForStaff,
      ...
      next: (updated) => {
        ...
        this.driverStartLatitudeSelection[updated.id] =
          updated.driverStartLatitude && updated.driverStartLatitude !== 0 ? updated.driverStartLatitude : null;
        this.driverStartLongitudeSelection[updated.id] =
          updated.driverStartLongitude && updated.driverStartLongitude !== 0 ? updated.driverStartLongitude : null;
      },
      error: (err) => {
        this.loading.set(false);
        this.messageService.add({ severity: 'error', summary: 'Error', detail: err?.error?.message || err?.message || 'Failed to assign roles' });
      },
    });
  }
